...
async function v2() {
    ...
    // tmp = await _test(() => v._encrypt('data', key), r => /^aes-256-gcm:\w{32}:\w+$/.test(_log(r)), 'Result should match the regex!');
    ...
    // });
    // TODO: nur neue hash function verwendet => password
    ...
    // // search internal
    // _test(() => v._searchHash('Hallo'), r => !!_log(r), '!');
    ...
    // _test(() => v._getSearchWords(vData), r => (r && r.length == 3), 'Result should math the search fields length!');

    // // store / cache
    // _test(() => v._storeCache('1', vData), r => !!r, 'Result should not be false!');
    ...
    // _test(() => v._removeCache(['1']), r => !!r, 'Result should be true!');
    // _test(() => v._removeCache(['_1']), r => !!r, 'Result should be true!');
    ...
    // _test(() => v.get(_lastVid), r => _validateMap(r), 'Result should not be empty!');
    // _test(() => v.get(_lastVid, true), r => _validateMap(r), 'Result should not be empty!');
    ...
    // // publish
    // await _test(() => v.publish(vData, 'password', 5), r => _pushVid(_log(r)), 'Result should not be null!');
